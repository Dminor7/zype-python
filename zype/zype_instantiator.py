...
class ZypeClient(object):
    ...
    def _wrap_in_zype(self, data, *args, **kwargs):
    ...
    def _wrap_in_zype_executor(self, data, *args, **kwargs):
        request_kwargs = kwargs.pop("request_kwargs", self._request_kwargs)
        return ZypeClientExecutor(
            self._instatiate_api(),
            data=data,
            api_params=self._api_params,
            request_kwargs=request_kwargs,
    ...
    def __call__(self, *args, **kwargs):
        ...
        return self._wrap_in_zype_executor(
    ...
    def _get_client_from_name(self, name):
        if (
            ...
            return self._wrap_in_zype(data=self._data[name])
        ...
        if name in resource_mapping:
            ...
            return self._wrap_in_zype(url, resource=resource)
...
class ZypeClientExecutor(ZypeClient):
    def __init__(self, api, *args, **kwargs):
        super(ZypeClientExecutor, self).__init__(api, *args, **kwargs)

    def __getitem__(self, key):
        raise Exception(
            "This operation cannot be done on a" + " ZypeClientExecutor object"
        )

    def __iter__(self):
        raise Exception("Cannot iterate over a ZypeClientExecutor object")

    def __getattr__(self, name):
        if name.startswith("to_"):  # deserializing
            return self._api._get_to_native_method(name, self._data)
        return self._wrap_in_zype_executor(getattr(self._data, name))

    def __call__(self, *args, **kwargs):
        return self._wrap_in_zype(self._data.__call__(*args, **kwargs))
    ...
    def _make_request(self, request_method, refresh_token=None, *args, **kwargs):
        ...
        try:
            ...
            zype_exception = e.zype_exception(message=error_message, client=client)

            should_refresh_token = (
                refresh_token is not False and self._refresh_token_default
            )
            auth_expired = self._api.is_authentication_expired(zype_exception)
            ...
            if propagate_exception:
                raise zype_exception

        return self._wrap_in_zype(
    ...
    def __dir__(self):
        methods = [
            m for m in ZypeClientExecutor.__dict__.keys() if not m.startswith("_")
        ]
